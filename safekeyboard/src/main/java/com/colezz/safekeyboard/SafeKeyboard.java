...
/**
 * 安全键盘
 * 获得焦点清除内容； 禁止复制粘贴、禁止光标选择（始终指向最后一位）；
 * @author zengchi
 * @date 2022/5/20 11:41
 */
public class SafeKeyboard {
    ...
    private void initKeyboardContainer() {
        ...
        header.findViewById(R.id.keyboard_header_complete).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                hideKeyboard();
                if (mEditText instanceof LoginPassEditText) {
                    if (((LoginPassEditText) mEditText).getPassword() == null) {
                        return;
                    }
                    Log.d(TAG, "PassWord: " + ((LoginPassEditText) mEditText).getPassword().toString());
                    byte[] b = PayPassEncryptedUtil.getInstance().decryptedLoginPwd(((LoginPassEditText) mEditText).getPassword());
                    Log.d(TAG, "PassWordVisibleChar: " + b);
                    if (b.length >0) {
                        Log.d(TAG, "PassWordVisibleStr: " + Character.toString((char) b[0]));
                    }
                }
    ...
    private void disableCopyAndPaste(final EditText editText) {
        try {
            ...
            editText.setLongClickable(false); //禁止长按复制粘贴
        ...
        public void onKey(int primaryCode, int[] keyCodes) {
            ...
            if (primaryCode == Keyboard.KEYCODE_CANCEL) { // 收起
                ...
                if (editable != null && editable.length() > 0) {
                    if (start > 0) {
                        if (safeKeyboard.mEditText instanceof LoginPassEditText) {
                            ((LoginPassEditText) safeKeyboard.mEditText).deletePass(start - 1, start);
                        }
...
//                }
                ...
                if (editable != null && editable.length() > 0) {
                    if (safeKeyboard.mEditText instanceof LoginPassEditText) {
                        ((LoginPassEditText) safeKeyboard.mEditText).clearPass();
                    }
                    editable.clear();
                }
                ...
                Log.e(TAG, "primaryCode: " + primaryCode + ", keyCodes:" + keyCodes);
                Log.e(TAG, Character.toString((char) primaryCode));
                if (safeKeyboard.mEditText instanceof LoginPassEditText) {
                    ((LoginPassEditText) safeKeyboard.mEditText).insetPass(Character.toString((char) primaryCode));
                }
